        ...
        <div class="issue-main" v-if="lang === 'zh-CN'">
            <Card shadow>
                <div class="lkui-article">
                    <h2>在你开始之前...</h2>
                    <p>
                        首先，虽然我们为了中文用户的方便提供了中文的表单，但在填写时请尽量使用英文。LKCat 社区不只有中国人。如果你想让尽可能多的人能够看懂你的 issue，就请尽量用英文。
                    </p>
                    <p>
                        其次，LKCat 的 issue 列表只接受 bug 报告或是新功能请求 (feature requests)。这意味着我们不接受用法问题。如果你开的 issue 不符合规定，它将会被立刻关闭。<a
                    ...
                    <ul>
                        <li>仔细阅读 <a href="https://www.lkcatui.com/docs/guide/install" target="_blank">文档</a></li>
                        <li>在 <a href="https://stackoverflow.com/" target="_blank">StackOverflow</a> (英文) 或是 <a href="https://segmentfault.com/t/lkcat" target="_blank">SegmentFault</a>（中文）搜索和提问</li>
                    </ul>
                    <p>
                ...
                <div style="margin-top: 15px">
                    <Form ref="form" :model="formData" :rules="formRule" label-position="top">
                            ...
                            <Col span="12">
                                <FormItem label="相关仓库：" prop="repo">
                                    <Select v-model="formData.repo">
                                        <Option value="lkcat">LKCat</Option>
                                        <Option value="lkcat-admin">LKCat Admin</Option>
                                    </Select>
                                </FormItem>
                            </Col>
                            <Col span="12">
                        ...
                        <template v-if="formData.type === 'feature-request'">
                            ...
                            <div class="lkui-article" style="margin-bottom: 20px">
                                <p>
                                    请尽可能详尽地说明这个需求的用例和场景。最重要的是：解释清楚是怎样的用户体验需求催生了这个功能上的需求。LKCat 的一个重要设计原则是保持 API 的简洁和直接。通常来说，我们只考虑添加在现有的 API 下无法轻松实现的功能。新功能的用例也应当足够常见。
                                </p>
                            </div>
                ...
                <div class="lkui-article">
                    <p>
                        维护开源项目，尤其是流行的项目，是<a
                            ...
                            target="_blank">非常辛苦的工作</a>。随着 LKCat 在社区越来越受欢迎，我们每天都在收到越来越多的问题， bug 报告，功能需求和 Pull Requests。
                </p>
                    <p>
                        作为一个完全免费使用的开源项目，LKCat 的维护人手是有限的。这意味着想要让项目长期的可持续发展，我们必须：
                    </p>
                    <ul>
            ...
            <Modal v-model="isMin" class-name="issue-vertical-center-modal" title="关于重现" width="680">
                <div class="lkui-article">
                    ...
                    <ul>
                        ...
                        <li>由于涉及了大量业务代码，问题可能是你的代码错误，而不是 LKCat 的 bug 所导致的。</li>
                    </ul>
                    <p>
            ...
            <Modal v-model="isPreview" title="Issue 预览" width="680">
                <div class="lkui-article" v-if="isPreview" v-html="content.html"></div>
                <div slot="footer">
        ...
        <div class="issue-main" v-else>
            <Card shadow>
                <div class="lkui-article">
                    ...
                    <ul>
                        <li>Read the <a href="https://www.lkcatui.com/docs/guide/install-en" target="_blank">docs</a></li>
            ...
            <Modal v-model="isStrict" class-name="issue-vertical-center-modal" title="The reason behind our strict issue policy" width="680">
                <div class="lkui-article">
                    <p>
                        Maintaining open source projects, especially popular ones, is <a href="https://nolanlawson.com/2017/03/05/what-it-feels-like-to-be-an-open-source-maintainer/" target="_blank">hard work</a>. As LKCat's user base has grown, we are getting more and more usage questions, bug reports, feature requests and pull requests every single day.
                    </p>
                    <p>
                        As a free and open source project, LKCat also has limited maintainer bandwidth. That means the only way to ensure the project's sustainability is to:
                    </p>
                    <ul>
            ...
            <Modal v-model="isMin" title="About Reproductions" width="680">
                <div class="lkui-article">
                    ...
                    <ul>
                        ...
                        <li>The problematic behavior may very well be caused by your code rather than by a bug in LKCat.</li>
                    </ul>
                    <p>
...
<script>
    ...
    function generate (markdown) {
        const withMarker = markdown + '\n\n<!-- generated by lkcat-issues. DO NOT REMOVE -->';
...
${this.formData.api}`.trim())
            ...
            handleCreate () {
                ...
                window.open(`https://github.com/lkcat/${this.formData.repo}/issues/new?title=${title}&body=${body}`);
            }
        },
        created () {
            this.lang = this.$lang;
